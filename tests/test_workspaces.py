"""Workspace store v2: CRUD + blob-split round-trips over the HTTP router.

No remote calls — the `client` fixture (conftest) stubs the tinker capabilities
probe and points state at a tmp dir, so this exercises only the `/api/workspaces`
router + `api/workspace_store.py` (per-workspace files, write-once node blobs,
...
def test_list_returns_summaries_without_trees(client):
    """GET /api/workspaces (default) returns summaries — id/name/timestamps/panels,
...
def test_bodies_query_returns_light_trees(client):
    ...
    client.post("/api/workspaces", json={"name": "B", "trees": {"primary": _heavy_tree()}})
    bodies = client.get("/api/workspaces?bodies=1").json()
...
def test_get_one_body(client):
    cid = client.post("/api/workspaces", json={"name": "one", "trees": {"primary": _heavy_tree()}}).json()["id"]
...
def test_create_accepts_initial_trees(client):
    ...
    cid = client.post("/api/workspaces", json={"name": "seed", "trees": {"primary": tree}}).json()["id"]
...
def test_create_with_client_id_upserts(client):
    ...
    a = client.post("/api/workspaces", json={
    ...
    b = client.post("/api/workspaces", json={"id": cid, "name": "Draft2", "trees": {"primary": {"mark": "v2"}}}).json()
    assert b["id"] == cid
    listed = client.get("/api/workspaces").json()
    assert len(listed) == 1 and listed[0]["name"] == "Draft2"
    assert client.get(f"/api/workspaces/{cid}").json()["trees"] == {"primary": {"mark": "v2"}}
...
def test_node_blobs_fetch(client):
    cid = client.post("/api/workspaces", json={"name": "blob", "trees": {"primary": _heavy_tree()}}).json()["id"]
    got = client.post(f"/api/workspaces/{cid}/node-blobs", json={"nodes": ["n1", "unknown"]}).json()
...
def test_node_blobs_empty_for_light_node(client):
    ...
    cid = client.post("/api/workspaces", json={"name": "l", "trees": {"primary": tree}}).json()["id"]
    assert client.post(f"/api/workspaces/{cid}/node-blobs", json={"nodes": ["n2"]}).json() == {}
    node = client.get(f"/api/workspaces/{cid}").json()["trees"]["primary"]["nodes"]["n2"]
...
def test_blob_write_once_idempotent(client):
    ...
    cid = client.post("/api/workspaces", json={"name": "wo", "trees": {"primary": _heavy_tree()}}).json()["id"]
...
def test_save_tree_self_heals_migrated_legacy_shape_without_losing_compare(client):
    ...
    does NOT drop the compare tree (2 real workspaces depend on this)."""
    cid = client.post("/api/workspaces", json={"name": "L"}).json()["id"]
...
def test_patch_rename(client):
    cid = client.post("/api/workspaces", json={"name": "old"}).json()["id"]
    r = client.patch(f"/api/workspaces/{cid}", json={"name": "new"})
    assert r.status_code == 200 and r.json()["name"] == "new"
    assert client.get("/api/workspaces").json()[0]["name"] == "new"
...
def test_patch_layout_fields_without_trees(client):
    ...
    cid = client.post("/api/workspaces", json={"name": "c", "trees": {"primary": _heavy_tree()}}).json()["id"]
    layout = [{"id": "primary", "run_id": "run-x", "checkpoint": "step-9"}]
    r = client.patch(f"/api/workspaces/{cid}", json={
...
def test_patch_only_touches_provided_fields(client):
    cid = client.post("/api/workspaces", json={"name": "keep", "system_prompt": "SP"}).json()["id"]
    client.patch(f"/api/workspaces/{cid}", json={"name": "renamed"})
    body = client.get(f"/api/workspaces/{cid}").json()
...
def test_system_enabled_travels_with_the_conversation(client):
    ...
    legacy = client.post("/api/workspaces", json={"name": "old", "system_prompt": "sp"}).json()
    assert client.get(f"/api/workspaces/{legacy['id']}").json().get("system_enabled") is None
...
def test_delete_removes_file_and_blobs(client):
    import tinkerscope.api.workspace_store as store
    cid = client.post("/api/workspaces", json={"name": "c", "trees": {"primary": _heavy_tree()}}).json()["id"]
    assert store._ws_file(cid).exists() and store._blobs_dir(cid).exists()
    assert client.delete(f"/api/workspaces/{cid}").json()["status"] == "ok"
    assert client.get("/api/workspaces").json() == []
    assert not store._ws_file(cid).exists()
...
def test_corrupt_conversation_file_is_quarantined_not_fatal(client):
    """A single corrupt per-workspace file is moved aside on next cache build, and
    the other workspaces still load (one bad file can't nuke the store)."""
    import tinkerscope.api.workspace_store as store
    good = client.post("/api/workspaces", json={"name": "good"}).json()["id"]
    bad = client.post("/api/workspaces", json={"name": "bad"}).json()["id"]
    store._ws_file(bad).write_text("{ not valid json ,,,")
    ...
    backups = list(store._ws_dir().glob(f"{bad}.json.corrupt-*"))
...
def test_crafted_ids_cannot_escape_the_store(client, tmp_path):
    """A workspace/node id becomes a path component; a crafted id must not read or
    ...
    assert client.get("/api/workspaces/..%2F..%2Fsecret").status_code in (404, 400)
    # A crafted CREATE id is rejected with a clean 400 (not a 500 from the write path).
    assert client.post("/api/workspaces", json={"id": "../evil", "name": "x"}).status_code == 400

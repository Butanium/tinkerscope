...
def test_claim_sets_workspace_and_layout_together():
    st = _apply(conversation_id="ws-a", panels=_panels(("primary", "run-a")))
    assert st.conversation_id == "ws-a"
    assert [p.run_id for p in st.panels] == ["run-a"]


def test_foreign_incremental_write_cannot_graft_onto_the_bus():
    _apply(conversation_id="ws-a", panels=_panels(("primary", "run-a")))
    # Tab B (workspace ws-b) echoes ITS transcript for a panel id that collides.
    st = _apply(conversation_id="ws-b", panel_messages={"primary": [{"role": "user", "content": "from B"}]})
    assert st.conversation_id == "ws-a", "the stamp must not move without a claim"
    assert st.panels[0].messages == [], "ws-b's echo must not land on ws-a's panel"


def test_foreign_single_panel_subpatch_is_dropped():
    _apply(conversation_id="ws-a", panels=_panels(("primary", "run-a")))
    st = _apply(conversation_id="ws-b", panel="primary", run_id="run-b", checkpoint="step-3")
    assert [p.run_id for p in st.panels] == ["run-a"]
    assert st.panels[0].checkpoint == "final"


def test_foreign_system_prompt_is_dropped():
    _apply(conversation_id="ws-a", system_prompt="A's prompt", system_enabled=True)
    st = _apply(conversation_id="ws-b", system_prompt="B's prompt", system_enabled=False)
...
def test_foreign_patch_still_applies_GLOBAL_params():
    ...
    _apply(conversation_id="ws-a", panels=_panels(("primary", "run-a")))
    st = _apply(conversation_id="ws-b", temperature=0.25, n_samples=8,
...
def test_foreign_patch_WITH_panels_claims_the_bus():
    _apply(conversation_id="ws-a", panels=_panels(("primary", "run-a")))
    st = _apply(conversation_id="ws-b", panels=_panels(("primary", "run-b"), ("p-2", "run-c")))
    assert st.conversation_id == "ws-b"
...
def test_stamped_patch_applies_when_no_workspace_is_on_the_bus_yet():
    ...
    st = _apply(conversation_id="ws-a", system_prompt="hello")
    assert st.conversation_id == "ws-a"
...
def test_own_incremental_write_applies():
    _apply(conversation_id="ws-a", panels=_panels(("primary", "run-a")))
    st = _apply(conversation_id="ws-a", panel_messages={"primary": [{"role": "user", "content": "mine"}]})
...
async def test_chat_begin_goes_through_the_same_guard():
    ...
    _apply(conversation_id="ws-a", panels=_panels(("primary", "run-a")))
    await BUS.chat_begin(conversation_id="ws-b", panel="primary", run_id="run-b",
                         messages=[{"role": "user", "content": "hi"}])
    assert [p.run_id for p in BUS.state.panels] == ["run-a"]
    assert BUS.state.conversation_id == "ws-a"
